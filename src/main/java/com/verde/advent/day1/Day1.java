 ...
 */
public class Day1 {
    public static int compute(String input, int skip) throws RuntimeException {
        // We need to handle the circular aspect, so tack the entire input back onto itself
        String s = input + input;

        int sum = 0;
        for (int i=0; i<input.length(); ++i) {
            if (s.charAt(i) == s.charAt((i+skip) % input.length())) {
    ...
    public static int computeSkipOne(String input) throws RuntimeException {
        return compute(input, 1);
    }
    
    public static int computeSkipHalf(String input) throws RuntimeException {
        return compute(input, input.length() / 2);
    }
}
