 ...
 * immediately after the opcode tell you these three positions - the first two indicate the positions from which you should read 
 ...
 * the positions of the two inputs (9 and 10), and the position of the output (3). To handle this opcode, you first need to get 
 * the values at the input positions: position 9 contains 30, and position 10 contains 40. Add these numbers together to get 70. 
 ...
 * 
 * Opcodes (like 1, 2, or 99) mark the beginning of an instruction. The values used immediately after an opcode, if any, are 
 * called the instruction's parameters. For example, in the instruction 1,2,3,4, 1 is the opcode; 2, 3, and 4 are the parameters. 
...
public class Y2019D02 {
    public static int runProgram(int[] inputs) {
        return runProgram(inputs, -1, -1);
    }
        
    public static int runProgram(int[] inputs, int fixGravityAssistNoun, int fixGravityAssistVerb) {
        // Use a copy of the data, since we don't know if the inputs will be used in the future
        inputs = Arrays.copyOf(inputs, inputs.length);

        // If repairing gravity assist, then apply the required changes
        if ((fixGravityAssistNoun >= 0) && (fixGravityAssistVerb >= 0)) {
            inputs[1] = fixGravityAssistNoun;
            inputs[2] = fixGravityAssistVerb;
        }
        
        boolean done = false;
        for (int opCodeIndex = 0; !done; opCodeIndex += 4) {
            int opCode = inputs[opCodeIndex];
            
            if ((opCode == 1) || (opCode == 2)) { 
                int lhsRef = inputs[opCodeIndex + 1];
                int rhsRef = inputs[opCodeIndex + 2];
                int resultRef = inputs[opCodeIndex + 3];
                
                inputs[resultRef] = 
                        (opCode == 1) 
                        ? inputs[lhsRef] + inputs[rhsRef] 
                        : inputs[lhsRef] * inputs[rhsRef];
            } else if (opCode == 99) {
                done = true;
            }
